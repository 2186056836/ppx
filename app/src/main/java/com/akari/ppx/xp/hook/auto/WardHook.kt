...
class WardHook : SwitchHook("auto_ward") {
    override fun onHook() {
        ...
        when (condition) {
            ...
            /* 为帖子评论时 */ 1 -> {
            "com.sup.android.module.publish.publish.c".hookBeforeMethod(
                cl,
                "a",
                "com.sup.android.mi.publish.bean.PublishBean"
            ) { param ->
                with(param.args[0]) {
                    if (callMethodOrNullAs<Int>("getType") == 2 && callMethodOrNullAs<Long>("getFakeId") != 666L) {
                        callMethod("setFakeId", 666L)
                        callMethodOrNullAs<Long>("getCellId")?.let(::wardCell)
                    }
                }
            }
        }
